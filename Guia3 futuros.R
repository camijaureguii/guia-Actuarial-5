...
#ejercicio 3
#El dividendo de $10 se paga a los 9 meses, fuera del periodo del forward (6 meses), asi que su PV dentro del contrato es 0.
a_precioforw_6m = 150*exp(0.12*0.5) #no resto los 10 ya que si compro forward los recibo
a_precioforw_6m

#b) tomo posicion vendida en el forward, compro accion hoy tomando prestamo, en 6 meses resto lo que me dan de vender el forward
#con lo que debo pagar de prestamo 
b_arbitraje_6m =(165-a_precioforw_6m)
b_arbitraje_6m

#c) #Si compras un forward a 12 meses no recibis nunca el dividendo, y el precio de hoy del activo lo contiene, hay que restarlo
c_precioforw_12m = (150-10*exp(-0.12*9/12))*exp(0.12) #o 150*exp(0.12)-10*exp(0.12*3/12)
c_precioforw_12m

#d) El de mercado esta caro. Compro lo barato, vendo lo caro. Vendo el de mercado. Me obligo a vender la accion
#al vencimiento. Entonces, hoy pido un prestamo para comprarla asi ya me aseguro tenerla al
#vencimiento y me cubro de los altibajos del precio de la accion en el mercado.
...
#si quiero ver cuantas acciones equivalen hoy:
Acciones_equiv_hoy = NF*multiplicador/(1+tasa_div)^0.5
Acciones_equiv_hoy

#si quiero ver cuantas equivalen en 6 meses
...
TNAr_trim = 0.08
TNAq_trim = 0.04 #accion paga dividendos
P= 10000
Bc =1.2
Bf = 0.95
Indice = 156
...
#a)Si quisiera eliminar el riesgo de mercado de la economia B, que posicion debera tomar?
...
#B) Como funcionara la cobertura si dentro de tres meses el valor del indice es 150 y el contrato de futuro cotiza a 25 x 151?
S3= 150*25
F3 =151*25

#Resultado por futuros a los 3 meses
Rdo_futuros = (F3-F0)*Nf #uso el Nf con coma para mostrar como la cobertura idea elimina completamente el beta.
Rdo_futuros
...
#??La cobertura funcion??? Queriamos eliminar el riesgo de mercado, o sea, neutralizar el efecto 
#de los movimientos del indice sobre tu cartera. Esto se traduce en: - Llevar el beta efectivo 
...
#C)??Podria eliminar tambien el riesgo de tipo de cambio? ??Por que nominal tomar??a cobertura? ??se eliminar??a totalmente?
#Mi cartera esta en moneda extranjera (economia B), pero yo mido resultados en mi moneda domestica.
#Entonces, estoy largo en la divisa extranjera, si la moneda extranjera se deprecia contra tu moneda, pierdo valor.
#Por ello, quiero cubrir la exposicion a que el tipo de cambio baje.
#Si tomas un short (venta) en el futuro, gano si la divisa extranjera baja.
...
#Por ello, abro un contrato de futuros en el mercado con posicion vendedora sobre un nominal de: 
VN_esperado = P*(1+rend_libre_riesgo)
VN_esperado
#Al vencimiento, el resultado es: Resultado=(hoy - Fvenc) N
#Si el tipo de cambio baja, venc < hoy , entonces tu posicion short gana, 
#compensando la perdida de la cartera en moneda extranjera.

#Aunque esta estrategia reduce el riesgo cambiario, no lo elimina completamente, ya que: 
#- Los betas usados son estimaciones teoricas. - El rendimiento real puede diferir de la tasa 
#libre de riesgo. - El valor nominal cubierto es una aproximacion esperada, no exacta.
